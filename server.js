...
const FRONTEND_ORIGINS = [
  process.env.FRONTEND_URL || 'https://hier-app.vercel.app',
  'http://localhost:3000',
  'http://127.0.0.1:3000'
];
...
const MONGO_URI =
  ...
  'mongodb+srv://anujkumar221203:AnujKumar9315266481@hier-major-project.7x8kzsj.mongodb.net/hierDB?retryWrites=true&w=majority';
...
function handleServerError(res, err, label = '') {
...
app.post('/register-customer', async (req, res) => {
  try {
    ...
    const customer = new Customer({ fullName, email, phone, password: hashedPassword });
    await customer.save();
    ...
    // Return JSON with redirectUrl so frontend can handle navigation
    res.status(201).json({
      success: true,
      message: 'Customer Registered',
      redirectUrl: '/views/driver-login.html'
    });
  } catch (err) {
    return handleServerError(res, err, 'REGISTER CUSTOMER ERROR:');
  }
});
...
app.post('/login-customer', async (req, res) => {
  try {
    ...
    return handleServerError(res, err, 'LOGIN CUSTOMER ERROR:');
  }
});
...
app.post('/register-driver', async (req, res) => {
  try {
    const { name, email, phone, licenseNumber, experience, city, password } = req.body;
    if (!name || !email || !phone || !licenseNumber || !experience || !city || !password) {
      return res.status(400).json({ success: false, error: 'All fields required' });
    }
    ...
    const hashedPassword = await bcrypt.hash(password, 10);
    const driver = new Driver({ name, email, phone, licenseNumber, experience, city, password: hashedPassword });
    await driver.save();
    ...
    return handleServerError(res, err, 'REGISTER DRIVER ERROR:');
  }
});
...
app.post('/login-driver', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      driver: { name: driver.name, email: driver.email, phone: driver.phone, city: driver.city, verified: driver.verified }
    });
  } catch (err) {
    return handleServerError(res, err, 'LOGIN DRIVER ERROR:');
  }
});
...
app.post('/forgot-password-customer', async (req, res) => {
  try {
    ...
    const resetLink = `${BASE_URL}/reset-password-customer/${token}`;
    ...
    return handleServerError(res, err, 'FORGOT PASSWORD ERROR:');
  }
});

// RECENT BOOKINGS (for drivers)
app.get('/recent-bookings', async (req, res) => {
  try {
    const bookings = await Booking.find().sort({ createdAt: -1 }).limit(5).lean();
    // convert _id to string for frontend convenience
...
// GLOBAL ERROR HANDLER (process-level)
...
app.listen(PORT, () => {
  console.log(`🚀 Server running on ${BASE_URL} (listening port ${PORT})`);
});
